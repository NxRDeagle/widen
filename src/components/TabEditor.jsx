...
const TabEditor = ({ messengerStates, setMessengerStates }) => {
  ...
  function addChat(chat) {
    setMessengerStates({
      ...
      activeTabChats: messengerStates.activeTabChats.includes(chat)
        ? messengerStates.activeTabChats.filter((activeTabChat) => activeTabChat !== chat)
  ...
  function includesSubscriber(subscriberId) {
    return messengerStates.activeTabChats.find(
      (activeTabChat) =>
        activeTabChat.companionsId.length === 1 &&
        activeTabChat.companionsId.includes(subscriberId),
    );
  }
  ...
  return (
    <>
      <div
        style={
          confirmationOpen
            ? { filter: 'blur(7.5px)', width: '100%', overflowY: 'hidden', pointerEvents: 'none' }
            : null
        }
        className={
          openTabEditor ? 'tab_editor_container' : 'tab_editor_container tab_editor_container_close'
        }>
        <svg
          onClick={() => {
            TabEditorBack();
        ...
        <button
          onClick={() =>
            alreadyTabEditor(
              messengerStates.tabsName,
              messengerStates.tabsName[messengerStates.tabPosition],
              messengerStates.activeTabChats,
            )
          }
                  ...
                  <img
                    ...
                    src={
                      item.conversationAvatar
                        ? item.conversationAvatar
                        : getUser(item.companionsId[0]).avatar
                    }
                    alt="avatar"></img>
                  <p className="add_chat_nickname">
                    {item.conversationName
                      ? item.conversationName
                      : getUser(item.companionsId[0]).nickname}
                  </p>
                  <div onClick={() => addChat(item)} className="delete_chat_box">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="26"
                      height="26"
        ...
        <div
          style={
            !messengerStates.activeTabChats.length ? { marginTop: '200px' } : { marginTop: '10px' }
          }
        ...
        <div className="tab_location_box">
          ...
          {!messengerStates.isNewTab && (
            <button
              onClick={() => TabAction(`Удалить вкладку "${signTab}"?`, 'deleteTab')}
              className="drop_tab_btn">

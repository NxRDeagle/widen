...
const Reply = ({ replyId, authorReplyId }) => {
  ...
  const { nickname, avatar } = users_data.find((obj) => obj.userId === authorReplyId);
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        setMessageText(`
          ${nickname}`);
        block.innerHTML = `<a onClick = {() => {
          goToPreview();
        }}
     
         style='color:var(--color_active)'>${nickname}</a>, `;
        block.focus();
      }}>
      ...
      <div className="reply_text_box">
        <p className="reply_user_nickname">{nickname}</p>
        <p className="reply_text">{reply.text}</p>
        <span className="time_reply_box">
          <p className="time" style={{ marginRight: '31px', fontSize: '8px' }}>
            число месяц в время
          </p>
        </span>
      </div>
      <div className="reply_like_box">
        <svg
          ...
          <path
            d="M4.575 1C2.60063 1 1 2.62303 1 4.62505C1 8.25009 5.225 11.5456 7.5 12.3121C9.775 11.5456 14 8.25009 14 4.62505C14 2.62303 12.3994 1 10.425 1C9.216 1 8.14675 1.60868 7.5 2.54032C7.17035 2.06418 6.7324 1.67561 6.22325 1.40748C5.71409 1.13936 5.14872 0.99959 4.575 1Z"
            fill="white"
            stroke="#888888"
            strokeLinecap="round"

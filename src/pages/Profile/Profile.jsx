...
const Profile = () => {
    ...
    const {
        ...
        editMyProfile, 
        profileActiveIcon, 
        setProfileActiveIcon } = React.useContext(mainContext);
    ...
    let subscriptions = itsMyProfile ? Conversion('count', myProfile.subscriptions.length) : Conversion('count', userProfile.subscriptions.length);
    let subscribers = itsMyProfile ? Conversion('count', myProfile.subscribers.length) : Conversion('count', userProfile.subscribers.length);
    ...
    return !userProfile ? (
        ...
        <>
            <div
                ...
                style={{ backgroundImage: `url(${itsMyProfile ? myProfile.background : userProfile.background})` }}>
                <Update />
                {
                    itsMyProfile ?
                        (
                ...
                <div className="profile_avatar">
                    <img loading='lazy' className="avatar_picture" src={itsMyProfile ? myProfile.avatar : userProfile.avatar} alt="avatar" />
                    {
                        !itsMyProfile && (
                            <div className={userProfile.online ? 'profile_status profile_status_online' : 'profile_status profile_status_offline'}>
                                {
                                    !userProfile.online && (
                                        <p className='profile_last_time'>{Conversion('time', new Date(userProfile.lastTime), page)}</p>
                                    )
                                }
                            </div>
                        )
                ...
                <div className="profile_box">
                    <div className="profile_user_count_container">
                        <ProfileStatistics count={subscriptions} onClick={() => console.log('Подписки')}>Подписки</ProfileStatistics>
                        <ProfileStatistics count={subscribers} onClick={() => console.log('Подписчики')}>Подписчики</ProfileStatistics>
                    </div>

                    <div className="profile_nick_box">
                        <h1 className="profile_nickname">{itsMyProfile ? myProfile.nickname : userProfile.nickname}</h1>
                        <p className="profile_fi">
                            {itsMyProfile ? `${myProfile.firstName} ${myProfile.lastName}` : `${userProfile.firstName} ${userProfile.lastName}`}
                        </p>
                            ...
                            itsMyProfile ?
                                ...
                                (
                                    <>
                                        ...
                                        <ProfileBtn onClick={() => editMyProfile(userProfile.userId, 'subcriptions')}>{myProfile.subscriptions.includes(userProfile.userId) ? "Отписаться" : "Подписаться"}</ProfileBtn>
                                    </>
                                )
                        }
                    </div>
                    ...
                    <div className={profileActiveIcon === 3 ? 'achievementItems' : 'achievementItems none_active'}></div>
                </div>
            </div>
        </>
    );

...
export function GlobalStates() {
  ...
  const appState = {
    ...
    fullImages: [''],
    fullImgIndex: 0,
    myProfile: users_data.find((obj) => obj.userId === 1)
      ? users_data.find((obj) => obj.userId === 1)
      : defaultUser,
    ...
    clickChat: localStorage.getItem('clickChat')
      ? JSON.parse(localStorage.getItem('clickChat'))
      : defaultChat,
    changeMsgs: [],
    editorMsg: false,
  ...
  //Создание Редюсера(оброботчика всех стейтов)
  ...
  appvalue.clickGlobalFilter = (
    globalSearchName = 'globalCase',
    filterSign = '',
    objGlobalFilter = 'tag',
  ) => {
  ...
  // Удаление всех фильтров в партнерствах
  appvalue.dropPartnershipFilters = () => {
    dispatch({ type: 'CLEAR_PARTNERSHIP_FILTERS' });
  };

  //Удаление тега определённой категории из партнерств
  appvalue.dropPartnershipFilter = (category = '', partnershipFilter = '') => {
    dispatch({
      type: 'DROP_PARTNERSHIP_FILTER',
      payload: { category: category, partnershipFilter: partnershipFilter },
    });
  };

  //Добавление фильтра нужной категории в партнерствах
  appvalue.addPartnershipFilter = (category = '', partnershipFilter = '') => {
    dispatch({
  ...
  appvalue.deleteMessages = () => {
    ...
    appvalue.changeMsgs.forEach(
      (changeMsg) =>
        (newClickChat.messages = newClickChat.messages.filter(
          (message) => message.messageId !== changeMsg.messageId,
        )),
    );
  ...
  appvalue.redactorMessage = () => {
    const newClickChat = { ...appvalue.clickChat };
    newClickChat.messages = newClickChat.messages.map((message) => {
      return message.messageId !== appvalue.changeMsgs[0].messageId
        ? message
        : { ...message, message: appvalue.messageText, edited: true };
    });

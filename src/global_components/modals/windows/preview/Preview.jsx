...
export default function Preview({ onClose, currentModal }) {
  const { userProfile, Conversion, modalOption, goToProfile, setModal } = useContext(mainContext);
  ...
  return (
    <div
      className={
        modalOption === 'preview' ? 'preview_box preview_box_open' : 'preview_box preview_box_close'
      }
      style={
        userProfile.background
          ? { backgroundImage: `url(${userProfile.background})` }
          : { backgroundColor: '#7e52ee' }
      }>

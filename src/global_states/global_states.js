...
export function GlobalStates() {
    ...
    appvalue.clearGlobalFilter = () => {
        dispatch({ type: 'CLEAR_GLOBAL_FILTERS' });
    };

    //Удаление тега определённой категории(теги, формат, формы и т.д) из глобального поиска
    appvalue.dropGlobalFilter = (category = '', value = '') => {
        dispatch({ type: "DROP_GLOBAL_FILTER", payload: { code: category, value: value } });
    };

    //Добавление фильтра нужной категории в глобальном поиске
    appvalue.addGlobalFilter = (category = '', value = '') => {
        dispatch({ type: "ADD_GLOBAL_FILTER", payload: { code: category, value: value } });
    };
